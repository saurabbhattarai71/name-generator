  ...
  <div class="min-h-screen flex flex-col items-center bg-gray-100 p-4">
    ...
    <div class="flex items-center space-x-2 mt-4">
      ...
      <button class="group relative inline-flex h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-neutral-950 font-medium text-neutral-200 transition-all duration-300 hover:w-32">
        <div class="inline-flex whitespace-nowrap opacity-0 transition-all duration-200 group-hover:-translate-x-3 group-hover:opacity-100">Generate</div>
    ...
    <div class="p-4 w-full flex">
      <div class="w-1/2 p-2">
        <div class="space-y-6"> 
            <div v-for="(Babyname, index) in leftNames" :key="index" class="flex items-center justify-between px-12 py-4 bg-white shadow rounded-lg">
                <div class="flex items-center w-full justify-between">
                    <div>
                        <span class="font-semibold font-[poppins] text-sm">{{ Babyname.name }}</span> - {{ Babyname.description }} [{{ Babyname.origin }}]
                    </div>
                    <div class="flex items-center space-x-2 ml-4">
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24">
                            <path fill="currentColor" d="M4 7v14h14v2H4c-1.1 0-2-.9-2-2V7zm16-4c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H8c-1.1 0-2-.9-2-2V5c0-1.1.9-2 2-2h3.18C11.6 1.84 12.7 1 14 1s2.4.84 2.82 2zm-6 0c-.55 0-1 .45-1 1s.45 1 1 1s1-.45 1-1s-.45-1-1-1m-4 4V5H8v12h12V5h-2v2z"/>
                        </svg>
                    </div>
                </div>
            </div>
    ...
    <button class="group relative h-12 w-48 overflow-hidden rounded-lg bg-white text-lg shadow">
      ...
      <span class="relative text-black group-hover:text-white">Generate More</span>
    </button>
  </div>
</template>
  ...
  data() {
    return {
      names: [
        ...
        { name: 'Richard', description: 'Strong power; hardy power', origin: 'French' },
        { name: 'Trent', description: 'Traveller; journey across', origin: 'English' },
        ...
        { name: 'Bryant', description: 'Noble, strong, virtuous', origin: 'Gaelic' }
      ]
    };
  },
  computed: {
